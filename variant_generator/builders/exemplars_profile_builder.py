...
class ExemplarsProfileBuilder:
    ...
    def build(self, input_dir: str, output_file_path: str) -> dict:
        ...
        if not profile:
            logger.error("Could not produce a exemplars profile draft")
            return {}
        ...
        try:
            ...
            logger.success(
                f"Autogenerated exemplars profile → {output_file_path} (loads cleanly, "
                f"{len(loaded.item_types)} item type(s): {', '.join(loaded.type_keys)})"
            )
        except Exception as e:
            logger.warning(
                f"Autogenerated exemplars profile → {output_file_path} — "
