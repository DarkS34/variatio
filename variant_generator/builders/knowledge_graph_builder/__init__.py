...
**A build writes exactly one ARTIFACT**, at `kg_autogenerated_path`, plus two derivations in
`cache/`: the corpus anchoring at `concept_sources_path` and the difficulty calibration at
`concept_difficulty_path`. Neither is an artifact — both are regenerated by a build and the
curated graph is hand-edited — so they do not count as going back to the chain of intermediate
stages that was removed. The old chaining (`knowledge_graph_partial.json` → `_cleaned.json`)
stays out, along with the paths that read it: intermediate files are not reintroduced as
"checkpoints", because a build is ~15 min and repeating it was judged cheaper than the
machinery.
...
BUILD_PHASES = (
    ...
    # One call per domain, like the one beside it, but without reasoning and over fewer
    # concepts — only the taggable ones — so it weighs considerably less. The weights are
    # shares and `progress._Overall` normalises by their sum, so adding this one rescales none
    # of the previous six by hand: each keeps its relative proportion.
