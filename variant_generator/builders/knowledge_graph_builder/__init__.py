...
modules that hand each other plain dicts in memory:
    ...
    curation.run(cleaned)  → curated: el artefacto, y sus derivaciones en caché

**A build writes exactly one ARTIFACT**, at `kg_autogenerated_path`, plus dos derivaciones
en `cache/`: el anclaje al corpus en `concept_sources_path` y la calibración de dificultad en
`concept_difficulty_path`. Ninguna de las dos es artefacto — se regeneran con una construcción
y el grafo curado se edita a mano —, así que no cuentan como volver a la cadena de etapas
intermedias que se quitó. El encadenado viejo
(`knowledge_graph_partial.json` → `_cleaned.json`) sigue fuera, con las rutas que lo leían:
no se reintroducen ficheros intermedios como «puntos de control», porque una construcción
son ~15 min y repetirla se juzgó más barato que la maquinaria.
...
BUILD_PHASES = (
    ...
    # Una llamada por dominio, como la de al lado, pero sin razonar y sobre menos conceptos
    # —solo los etiquetables—, así que pesa bastante menos que ella. Los pesos son shares y
    # `progress._Overall` normaliza por su suma, de modo que añadir este no reescala a mano
    # ninguno de los seis anteriores: cada uno conserva su proporción relativa.
    ("difficulty", "Calibrando cuánto exige cada concepto", 8),
)
...
class KnowledgeGraphBuilder:
    ...
    def curate(
        ...
        return curation.run(
            ...
            difficulty_path or self.workspace.concept_difficulty_path,
            schema=self.schema,
            max_attempts=self.max_repair_attempts,
        )
