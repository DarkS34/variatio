...
modules that hand each other plain dicts in memory:
    ...
    curation.run(cleaned)  → curated: el artefacto, y el anclaje al corpus en caché

**A build writes exactly one ARTIFACT**, at `kg_autogenerated_path`, plus the corpus
anchoring at `concept_sources_path` — que es caché, no artefacto, y por eso no cuenta como
volver a la cadena de etapas intermedias. El encadenado viejo
