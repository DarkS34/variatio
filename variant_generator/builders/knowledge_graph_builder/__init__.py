...
modules that hand each other plain dicts in memory:
    ...
    curation.run(cleaned)  → curated, and the one file a build writes

**A build writes exactly one file.** The old staging chain
(`knowledge_graph_partial.json` → `_cleaned.json`) is gone, along with the paths that read
it back. Do not reintroduce intermediate files as "checkpoints": a build is ~15 min and
repeating it was judged cheaper than the machinery.
