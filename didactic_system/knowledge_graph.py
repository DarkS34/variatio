...
_DEFAULT_KG_PATH = Path(__file__).parent / "KG_data" / "kg_en.json"


class KnowledgeGraph:
    def __init__(self, path: Path = _DEFAULT_KG_PATH):
        ...
        self.concept_domain: dict[str, str] = {c: d for d, cs in self.concepts.items() for c in cs}
    ...
    def _subgraph(self, types: list[str]) -> nx.DiGraph:
        return nx.DiGraph((u, v, d) for u, v, d in self._graph.edges(data=True) if d["type"] in types)

    def learning_frontier(self, mastered: set[str]) -> list[str]:
        G_req = self._subgraph([self._rel["prerequisite"]])
        return sorted(
            node for node in G_req.nodes
    ...
    def visualize(self, figsize: tuple[int, int] = (22, 16)) -> None:
        ...
        EDGE_STYLES = {
            self._rel["prerequisite"]: {"style": "solid", "color": "#888888", "alpha": 0.5},
            self._rel["combines_with"]: {"style": "dashed", "color": "#2ca02c", "alpha": 0.7},
            self._rel["special_case"]: {"style": "dotted", "color": "#d62728", "alpha": 0.7},
        }

        domain_color = {d: DOMAIN_COLORS[i % len(DOMAIN_COLORS)] for i, d in enumerate(self.concepts)}
        ...
        for rel_type, style in EDGE_STYLES.items():
            edges = [(u, v) for u, v, d in self._graph.edges(data=True) if d["type"] == rel_type]
            if edges:
                nx.draw_networkx_edges(
                    self._graph, pos, edgelist=edges, ax=ax,
                    style=style["style"], edge_color=style["color"],
                    alpha=style["alpha"], arrows=True,
                    arrowsize=15, connectionstyle="arc3,rad=0.08",
        ...
        nx.draw_networkx_labels(
            self._graph, pos, ax=ax,
            font_size=7, font_weight="bold", font_color="#222222",
        )
