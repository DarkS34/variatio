...
def review(
    ...
    modalities = _modalities_block(exemplars_profile)
    logger.info(
    ...
    with progress.step(
        "taggability", "Reviewing which concepts work as labels", len(domains)
    ) as reporter:
    ...
    progress.advance(1.0, f"{len(non_taggable)} concept(s) that are no use as labels")
...
def _judge_domain(
    ...
    prompt = prompts.review_taggable_concepts_prompt(
        ...
        modalities,
        _samples_block(exemplars_profile, exemplars_bank, members),
    )
