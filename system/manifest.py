...
class Manifest:
    REQUIRED_KEYS = ("content_context", "generation_rules", "primary_field", "fields")
    SCALAR_TYPES = {
        "string": str,
        "integer": int,
        "number": float,
    ...
    GUIDANCE_KEYS = ("extraction", "generation")
    ...
    def _validate_field_spec(cls, name: str, spec: dict) -> None:
        ...
        if guidance is not None:
            ...
            unknown = set(guidance) - set(cls.GUIDANCE_KEYS)
            if unknown:
                raise ValueError(
                    f"Field '{name}' 'guidance' has unknown keys: {sorted(unknown)} (allowed: {list(cls.GUIDANCE_KEYS)})"
                )
    ...
    def field_guidance(self, task: str) -> dict[str, str]:
        if task not in self.GUIDANCE_KEYS:
            raise ValueError(
                f"Unknown task '{task}'; expected one of {list(self.GUIDANCE_KEYS)}"
            )
        out: dict[str, str] = {}
    ...
    def _scalar(cls, t: str):
        if t not in cls.SCALAR_TYPES:
            raise ValueError(f"Unsupported scalar type: '{t}'")
        return cls.SCALAR_TYPES[t]
