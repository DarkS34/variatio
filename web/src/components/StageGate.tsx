      ...
      <div className="space-y-5">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {stage.is_autogenerated ? (
                <Badge
                  variant="outline"
                  title={t("stage.autogeneratedHint")}
                >
                  {t("stage.autogenerated")}
                </Badge>
              ) : null}
            </div>
          </div>
        ...
        {locked ? (
          <p className="flex items-center gap-1.5 text-small text-muted-foreground">
            ...
            {stage.approved_at
              ? t("stage.lockedOn", { when: when(stage.approved_at) })
              : t("stage.locked")}
          </p>
        ) : null}
          ...
          <>
            ...
            {waitingJob ? (
              ...
              <Alert tone="info" title={t("stage.buildingFirst")}>
                <p>{t("stage.buildingFirstBody", { label: stage.label })}</p>
              </Alert>
            ) : isRebuild(busyRun?.job?.kind) ? (
              <Alert tone="info" title={t("stage.rebuilding")}>
                <p>{t("stage.rebuildingBody", { label: stage.label })}</p>
              </Alert>
            ) : (
              <Alert tone="info" title={t("stage.patching")}>
                <p>{t("stage.patchingBody", { label: stage.label })}</p>
              </Alert>
            )}
