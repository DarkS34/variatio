...
export const es = {
  ...
  "cfg.pipelineNote": "Cada columna es una construcción y se lee de arriba abajo; cada parada, una llamada al modelo. Bajo el nombre, qué modelo la atiende — cada fase nombra el suyo y ninguna queda vacía; el círculo dice si razona antes de contestar y, mientras razona, el selector de al lado fija su esfuerzo. Razonar y una gramática no conviven en esta pila, así que encender una fase que hoy responde con gramática se la quita y deja la forma en manos del analizador y de la reparación. Tres nodos no dependen de un ajuste: el guardián no razona, el ejercicio lo decide cada encargo y la reparación es su propia gramática.",
