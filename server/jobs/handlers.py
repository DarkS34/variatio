...
# `MissingArtifactError` names the artifact and nothing else, which is right for a library
# and useless on a screen: only this layer knows a person is reading the job's error.
_TAGGABILITY_MISSING = {
    entrypoints.EXEMPLARS_PROFILE: (
        "La etiquetabilidad se decide contra el perfil de ejemplares, y esta asignatura "
        "no lo tiene todavía: constrúyelo antes."
    ),
    entrypoints.KNOWLEDGE_GRAPH: "No hay grafo de conocimiento que revisar.",
}
...
def _build(artifact: str):
    ...
    def handler(job: Job, control: JobControl) -> dict:
        ...
        ws = _workspace(job)
        working = approvals.working_path(ws, artifact)
        if working is not None:
            ...
            storage.backup(ws, approvals.canonical_path(ws, artifact), artifact)
        # What the slot holds NOW is what the build is about to read: a document dropped
        # in while it runs is one it never saw, and the record has to say so.
        documents = approvals.raw_documents(ws, approvals.RAW_SOURCE[artifact])
        try:
            ...
            raise
        approvals.record_built(ws, artifact, documents)
        approvals.retire_curated(ws, artifact)
        mirror.mirror_file(
            ws, approvals.autogenerated_path(ws, artifact) or approvals.canonical_path(ws, artifact)
        )
...
def handle_describe_concepts(job: Job, control: JobControl) -> dict:
    ...
    descriptions = entrypoints.describe_concepts(concepts=concepts, overwrite=overwrite, ws=ws)
...
def handle_tag(job: Job, control: JobControl) -> dict:
    ...
    bank = entrypoints.tag_bank(context, ids=ids)
...
HANDLERS = {
    "build_profile": _build(approvals.EXEMPLARS_PROFILE),
    "build_kg": _build(approvals.KNOWLEDGE_GRAPH),
    "build_bank": _build(approvals.EXEMPLARS_BANK),
