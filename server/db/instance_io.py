...
from .models import CURATED, DRAFT, SLOT_CORPUS, SLOT_EXEMPLARS

KINDS = (stages.KNOWLEDGE_GRAPH, stages.EXEMPLARS_PROFILE, stages.EXEMPLARS_BANK)


def _paths(ws: FsWorkspace) -> dict[str, dict[str, Path]]:
    return {
        ...
        # The bank has no draft of its own: what a build writes is the only version there
        # is, and editing it in the UI overwrites the same file.
...
def import_instance(
    ...
    imported: list[str] = []
    for kind, stage_paths in _paths(ws).items():
