...
from study.api import admin as study_admin
from study.api import jobs as study_jobs
...
def test_a_comparison_stocked_from_the_panel_has_no_evaluator():
    ...
    assert study_jobs.evaluator_of(job) is None
...
def test_a_comparison_somebody_commissioned_for_themselves_belongs_to_them():
    ...
    assert study_jobs.evaluator_of(job) == 7
...
def test_the_panel_marks_every_job_it_launches_as_stock():
    ...
    assert study_admin.STOCK_PARAM == "stock"
...
def test_the_administrator_who_ordered_it_cannot_answer_it(db):
    ...
    with pytest.raises(HTTPException) as raised:
        study_router._require(session, "stock0000001", _access(users["admin"], users["workspace"]))
...
def test_an_administrator_cannot_answer_somebody_elses_session(db):
    ...
    with pytest.raises(HTTPException) as raised:
        study_router._require(session, copy.id, _access(users["admin"], users["workspace"]))
    assert raised.value.status_code == 404
...
def test_the_evaluator_it_was_assigned_to_may_answer_it(db):
    ...
    row = study_router._require(session, copy.id, _access(users["nombre"], users["workspace"]))
...
def test_an_administrator_may_answer_a_set_once_it_is_assigned_to_them(db):
    ...
    row = study_router._require(session, copy.id, _access(users["admin"], users["workspace"]))
    assert row.id == copy.id
...
def test_the_unassigned_source_is_not_listed_as_holding_itself(db):
    session, users = db
    listing = study_admin.sets(workspace="default", db=session)
...
def test_an_assignment_is_what_makes_a_holder(db):
    ...
    holders = study_admin.sets(workspace="default", db=session)["sets"][0]["holders"]
