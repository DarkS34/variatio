...
from study.arms import system as system_arm
...
def _run_capturing(monkeypatch) -> list[Commission]:
    ...
    def fake_arm(arm, commission, context):
        ...
        from study import FAILED, ArmResult

        return ArmResult(arm, FAILED, None, "", "", "", "", [], 0)

    monkeypatch.setattr(study_run, "_validate", lambda *a: None)
    monkeypatch.setattr(study_run, "_screen", lambda *a: None)
    monkeypatch.setattr(study_run, "_tag", lambda *a: None)
    monkeypatch.setattr(study_run, "_safe_run", fake_arm)
    return seen


def test_an_empty_setting_means_the_same_model_that_writes_a_generation(offered, monkeypatch):
    _local_model(monkeypatch, None)
    assert study_config.LOCAL_MODEL == "el-rapido"
...
def test_the_setting_names_the_writer_whether_offered_or_not(offered, monkeypatch):
    ...
    assert study_config.LOCAL_MODEL == "el-de-la-instalacion"
...
def test_the_installations_writer_reaches_every_arm_on_the_commission(offered, monkeypatch):
    ...
    session = study_run.evaluate(_Context(), concepts=["Función"], seed=7)
    assert len(seen) == 3
