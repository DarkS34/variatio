...
def test_the_naive_prompt_carries_the_subject_context_whole(language):
    prompt = evaluation_prompts.of(language).naive_generation_prompt(
...
def test_an_empty_block_adds_no_section(language):
    render = evaluation_prompts.of(language).naive_generation_prompt
    common = dict(
        subject="Programación",
        educational_level="",
...
def test_the_rag_prompt_inherits_it_through_the_naive_one(language):
    prompts = evaluation_prompts.of(language)
