...
What it retrieves over is `evaluation.raw_text`: the raw slots read with a plain extractor
...
def run(commission: Commission, context) -> ArmResult:
    ...
    theory = retrieve(ws, CORPUS, query, evaluation_config.RAG_TOP_K_THEORY)
    exercises = retrieve(ws, EXEMPLARS, query, evaluation_config.RAG_TOP_K_EXERCISES)
    ...
    prompt = evaluation_prompts.of(context.language).rag_generation_prompt(
