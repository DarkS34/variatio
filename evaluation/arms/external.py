...
def primary() -> tuple[str, str]:
    ...
    chain = configured_providers() or evaluation_config.EXTERNAL_PROVIDERS
    if not chain:
        return "none", ""
...
def _gemini(prompt: str, model: str, key: str) -> str:
    ...
    response = httpx.post(
        ...
        timeout=evaluation_config.EXTERNAL_TIMEOUT,
    )
...
# `evaluation_config.EXTERNAL_PROVIDERS`, never this. Declared after the callers it names.
